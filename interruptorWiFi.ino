...
void handleUpdate() {
  ...
  for ( uint8_t i = 0; i < server.args(); i++ ) {
    ...
    if (server.argName(i) == "led") {
      ...
      needSaveConfig = true;
    } else if (server.argName(i) == "mqttEnable") {
      boardState.mqtt.enable = (atoi(server.arg(i).c_str()) == 0) ? false : true;
      needSaveConfig = true;
    } else if (server.argName(i) == "mqttIP") {
      boardState.mqtt.ip = server.arg(i);
      needSaveConfig = true;
    } else if (server.argName(i) == "mqttUser") {
      boardState.mqtt.user = server.arg(i);
      needSaveConfig = true;
    } else if (server.argName(i) == "mqttPassword") {
      boardState.mqtt.password = server.arg(i);
      needSaveConfig = true;
    }
  }
